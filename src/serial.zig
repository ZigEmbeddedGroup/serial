...
pub fn receiveBufferCount(port: std.Io.File) !usize {
    switch (builtin.os.tag) {
        .windows => {
...
pub fn transmitBufferCount(port: std.Io.File) !usize {
    switch (builtin.os.tag) {
        .windows => {
            var ret_error: std.os.windows.DWORD = 0;
            var ret_comstat: COMSTAT = std.mem.zeroes(COMSTAT);
...
fn openTestSerialDeviceFile(io: std.Io) !std.Io.File {

    // if any, these will likely exist on a machine
    const tty = switch (builtin.os.tag) {
        .windows => "\\\\.\\COM3",
        .linux => "/dev/ttyUSB0",
        .macos => "/dev/cu.usbmodem101",
        else => unreachable,
    };
    return std.Io.Dir.openFileAbsolute(io, tty, .{ .mode = .read_write }) catch |err| switch (err) {
...
test "basic configuration test" {
    ...
    const port = try openTestSerialDeviceFile(std.testing.io);
