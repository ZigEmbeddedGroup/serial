...
pub fn build(b: *std.Build) void {
    ...
    const serial_mod = b.addModule("serial", .{
        .root_source_file = .{ .src_path = .{ .owner = b, .sub_path = "src/serial.zig" } },
    });

    const echo_exe = b.addExecutable(.{
        .name = "serial-echo",
        .root_source_file = .{ .src_path = .{ .owner = b, .sub_path = "examples/echo.zig" } },
        .target = target,
        .optimize = optimize,
    });
    ...
    const list_exe = b.addExecutable(.{
        .name = "serial-list",
        .root_source_file = .{ .src_path = .{ .owner = b, .sub_path = "examples/list.zig" } },
        .target = target,
        .optimize = optimize,
    });
    ...
    if (os_tag == .windows) {
        const port_info_exe = b.addExecutable(.{
            ...
            .root_source_file = .{ .src_path = .{ .owner = b, .sub_path = "examples/list_port_info.zig" } },
            .target = target,
            .optimize = optimize,
        });
